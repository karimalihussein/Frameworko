...
return static function(Router $router, ContainerInterface $container) {
    ...
    $router->map('GET', '/auth/register', [\App\Http\Controllers\Auth\RegisterController::class, 'index'])->middleware(new \App\Http\Middleware\RedirectIfAuthenticatedMiddleware());
    $router->map('POST', '/auth/register', [\App\Http\Controllers\Auth\RegisterController::class, 'store']);
    $router->map('GET', '/auth/login', [\App\Http\Controllers\Auth\LoginController::class, 'index'])->middleware(new \App\Http\Middleware\RedirectIfAuthenticatedMiddleware());
